class Simulation {
  ...
  update() {
    ...
    this.ctx.fillStyle = `rgba(20,20,20,.3)`;
  ...
  get yearsPerFrame() {
    return this._yearsPerFrame || 1.8;
  }}
...
class Body {
  ...
  solve() {
    ...
    this.velocity.add(this.acceleration.scaleNew(this.yearsPerFrame));
...
let mouseBody = new Body(new Vector(0, 0), new Vector(0, 0), 3, 50, true);
let bodyIndex = null;
...
for (i = 0; i < 200; i++) {
  ...
  const a = new Vector(p.y, -p.x).normalise();
  const rf = Math.random();
  sim.addBody(new Body(p, a, 0.000003 + rf * 0.000003, 2 + rf * 10));
}
